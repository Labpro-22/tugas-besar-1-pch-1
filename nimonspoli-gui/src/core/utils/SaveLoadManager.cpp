...
void SaveLoadManager::saveProperties(ofstream& out, const GameState& state){
    ...
    for(int i = 0; i< board->getSize(); i++){
        ...
        if (t && t->getProperty()) propTiles.push_back(t);
    }
    // <JUMLAH_PROPERTI>
    out << propTiles.size() << "\n";
...
void SaveLoadManager::loadPlayers(std::ifstream& in, GameState& state) {
    ...
    for (auto* p : players) {
        ...
        // Status
        if      (tok[3] == "ACTIVE")   p->setStatus(PlayerStatus::ACTIVE);
        else if (tok[3] == "JAILED")   p->setStatus(PlayerStatus::JAILED);
        else if (tok[3] == "BANKRUPT") p->setStatus(PlayerStatus::BANKRUPT);
...
void SaveLoadManager::loadProperties(std::ifstream& in, GameState& state) {
    ...
    if (!board) {
        ...
        for (int i = 0; i < count; i++) std::getline(in, line);
        return;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        // Restore owner
        if (owner == "BANK") prop->clearOwner();
        else                 prop->setOwner(owner);
        ...
        if (auto* sp = dynamic_cast<StreetProperty*>(prop)) {
            ...
            if (bldStr == "H") {
                ...
                for (int b = 0; b < 4; b++) sp->buildHouse();
                sp->upgToHotel();
            } else {
                int bldCount = std::stoi(bldStr);
                for (int b = 0; b < bldCount; b++) sp->buildHouse();
            }
        }
    }
}
...
void SaveLoadManager::loadDeck(std::ifstream &in, GameState &state)
{
    ...
    for (auto& t : drawTypes) {
        ...
        SkillCard* card  = CardFactory::createSkillCard(type, value, duration);
        if (card) newDrawPile.push_back(card);
    }
    ...
    for (auto& t : discardTypes) {
        ...
        int    duration  = ctok.size() > 2 ? std::stoi(ctok[2]) : 0;
        SkillCard* card  = CardFactory::createSkillCard(type, value, duration);
        if (card) newDiscardPile.push_back(card);
    }
...
void SaveLoadManager::loadLogs(std::ifstream& in, GameState& state) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (logger) logger->addLog(turn, user, act, detail);
    }
}