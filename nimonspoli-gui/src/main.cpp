...
int main()
{
    ...
    // Init sementara untuk baca ukuran monitor
    InitWindow(1, 1, "");
    int monW = GetMonitorWidth(0);
    int monH = GetMonitorHeight(0);
    CloseWindow();

    // Window size = ukuran monitor (fullscreen-like tanpa flag fullscreen)
    GUIManager gui(monW, monH, title, 60);
    RenderTexture2D canvas = LoadRenderTexture(1920, 1080);
...
>>>>>>> origin/gs2
        ...
        DrawTexturePro(
            canvas.texture,
            {0, 0, 1920, -1080},              // source (flip Y)
            {0, 0, (float)winW, (float)winH}, // dest
            {0, 0}, 0, WHITE);
        EndDrawing();
    }
