...
static Property *getRealPropertyAt(GameMaster *gm, int tileIdx)
{
    if (gm == nullptr || tileIdx < 0)
        return nullptr;
    ...
    return propertyTile->getProperty();
}
...
void GameScreen::drawBuyDialog()
{
    ...
    Property *realProperty = nullptr;

    if (guiManager != nullptr)
    {
        gm = guiManager->getGameMaster();
    }

    if (gm != nullptr)
    {
        realPlayer = gm->getState().getCurrPlayer();
        realProperty = getRealPropertyAt(gm, buyDialog.tileIdx);
    }
    ...
    if (realPlayer != nullptr && realProperty != nullptr)
    {
        originalPrice = realProperty->getPurchasePrice();
        finalPrice = realPlayer->applyDiscount(originalPrice);
        balance = realPlayer->getBalance();
        discount = realPlayer->getDiscount();
    }
    else
    {
    ...
    bool realCanAfford = balance >= finalPrice;
    ...
    if (finalPrice < originalPrice)
    {
        std::string discountText =
            "Harga awal M" + std::to_string(originalPrice) +
            " | Diskon " + std::to_string(static_cast<int>(discount)) + "%";

        DrawText(discountText.c_str(), (int)(px + 20), (int)ry, 11, {100, 220, 120, 255});
        ry += 22;
    }
    else
    {
    ...
    int remaining = balance - finalPrice;
    ...
    bool buyHover = CheckCollisionPointRec(mouse, buyBtn) && realCanAfford;
    bool buyDisabled = !realCanAfford;
